...
with Database("http://127.0.0.1:8000", 'test', 'test', credentials=('user_db', 'user_db'), use_http=True) as db:
    ...
    # DEFINE INDEX userNameIndex ON TABLE user COLUMNS name FULLTEXT ANALYZER ascii BM25 HIGHLIGHTS;
    print(db.define_index("userNameIndex", "user").columns("name").search_analyzer("ascii").bm25().highlights())
    # DEFINE INDEX userNameIndex ON TABLE user COLUMNS name FULLTEXT ANALYZER ascii BM25 1.2 0.75;
    ...
    # DEFINE INDEX idx_hnsw_embedding ON TABLE Document FIELDS items.embedding HNSW DIMENSION 4 TYPE I64;
    print(db.define_index("idx_hnsw_embedding", "Document").fields("items.embedding").hnsw(4).type_i64())
